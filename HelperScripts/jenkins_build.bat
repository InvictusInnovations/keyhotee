...
IF EXIST "%INVICTUS_ROOT%\bin" (
  ...
  rmdir /Q /S "%INVICTUS_ROOT%\bin" || exit /b 7
)

echo Checking build directory
IF EXIST "%INVICTUS_ROOT%\build" (
  echo "Removing build directory"
  rmdir /Q /S "%INVICTUS_ROOT%\build" || exit /b 8
)
...
IF EXIST "%INVICTUS_ROOT%\packages" (
  ...
  rmdir /Q /S "%INVICTUS_ROOT%\packages" || exit /b 9
)

mkdir "%INVICTUS_ROOT%\packages" || exit /b 10
...
mkdir "%INVICTUS_ROOT%\build" || exit /b 11
pushd "%INVICTUS_ROOT%\build"
echo "Spawning cmake generator"
call "%INVICTUS_ROOT%\keyhotee\run_cmake.bat" -DBUILD_VERSION_PATCH=%BUILD_NUMBER% || exit /b 12
...
if not %MSBUILD_STATUS%==0 exit /b 13
cpack --verbose || exit /b 14

pushd "%INVICTUS_ROOT%/packages"
tar -czf Keyhotee_build_%BUILD_NUMBER%.tgz *.zip *.pdb || exit /b 15
